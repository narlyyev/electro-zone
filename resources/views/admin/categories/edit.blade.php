@extends('admin.layouts.app')
@section('content')
    <div class="row justify-content-center ">
        <div class="col-11">
            <div class="container-xl rounded my-3 p-3">
                <div class="h3">
                    Edit a category
                </div>
                <form action="{{ route('admin.categories.update', $category->id) }}" method="POST"
                      enctype="multipart/form-data">
                    @csrf
                    <div class="bg-white p-4 rounded-3">
                        <div class="row">
                            <div class="col-6 form-group mb-3">
                                <label for="parent_id" class="form-label">Parent<span class="text-danger">*</span></label>
                                <select name="parent_id" id="parent_id" class="form-control select2">
                                    <option selected value>No</option>
                                    @foreach($categories as $cat)
                                        <option value="{{ $cat->id }}" {{ $category->parent_id == $cat->id ? 'selected':'' }}>
                                            {{ $cat->getName() }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-6 mb-3">
                                <label for="sort_order">Sort Order<span class="text-danger">*</span></label>
                                <input type="number" class="form-control" id="sort_order" name="sort_order"
                                       value="{{ $category->sort_order }}">
                            </div>
                        </div>
                        <div class="mb-3">
                            <label for="attribute" class="form-label fw-semibold">
                                Attribute
                            </label>
                            <select id="attribute"
                                    class="form-select{{ $errors->has('attributes') ? ' is-invalid' : '' }} select2"
                                    name="attributes[]" multiple>
                                @foreach($attributes as $attribute)
                                    <option value="{{ $attribute->id }}" {{ $category->attributes->contains($attribute) ? 'selected' : '' }}>
                                        {{ $attribute->getName() }}
                                    </option>
                                @endforeach
                            </select>
                            @if($errors->has('attributes'))
                                <span class="invalid-feedback" role="alert">
                                <strong>{{ $errors->first('attributes') }}</strong>
                            </span>
                            @endif
                        </div>
                        <div class="row">
                            <div class="col-4 mb-3">
                                <label for="name" class="form-label">Name<span class="text-danger">*</span></label>
                                <input type="text" class="form-control" id="name" name="name" value="{{ $category->name }}">
                            </div>
                            <div class="col-4 mb-3">
                                <label for="name_ru" class="form-label">Name RU</label>
                                <input type="text" class="form-control" id="name_ru" name="name_ru"
                                       value="{{ $category->name_ru }}">
                            </div>
                            <div class="col-4 mb-3">
                                <label for="name_en" class="form-label">Name EN</label>
                                <input type="text" class="form-control" id="name_en" name="name_en"
                                       value="{{ $category->name_en }}">
                            </div>
                        </div>
                    </div>

                    <div class="bg-white p-4 my-3 rounded-3">
                        <div class="mb-3">
                            <label for="small_image" class="form-label">Kiçi surat</label>
                            <input class="form-control" type="file" id="small_image" name="small_image" data-default-file="{{ asset('storage/img/categories/small_images' . '/' . $category->small_image) }}">
                        </div>
                        <div class="mb-3">
                            <label for="big_image" class="form-label">Uly surat</label>
                            <input class="form-control" type="file" id="big_image" name="big_image" data-default-file="{{ asset('storage/img/categories/big_images' . '/' . $category->big_image) }}">
                        </div>
                    </div>
                    <button type="submit" class="btn btn-primary px-4">
                        Submit
                    </button>
                </form>
            </div>
        </div>
    </div>

    <script>
        document.addEventListener('DOMContentLoaded', function () {
            // Define an array of objects with information about buttons and images
            var slidersData = [
                { buttonId: 'deleteImageButtonSmallImage', currentImageId: 'currentImageSmallImage', route: '{{ route("admin.categories.deleteImageSmallImageFromEdit", $category->id) }}' },
                { buttonId: 'deleteImageButtonBigImage', currentImageId: 'currentImageBigImage', route: '{{ route("admin.categories.deleteImageBigImageFromEdit", $category->id) }}' }
                // Add more objects if needed
            ];

            // Iterate over the array using for...of loop
            for (const sliderData of slidersData) {
                let deleteButton = document.getElementById(sliderData.buttonId);
                let currentImage = document.getElementById(sliderData.currentImageId);

                if (deleteButton) {
                    deleteButton.addEventListener('click', function () {
                        // Make an AJAX request to delete the image
                        fetch(sliderData.route, {
                            method: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                'Content-Type': 'application/json',
                                'Accept': 'application/json'
                            },
                        })
                            .then(response => {
                                if (!response.ok) {
                                    throw new Error('Network response was not ok');
                                }
                                return response.json();
                            })
                            .then(data => {
                                // If the deletion was successful, update the UI
                                currentImage.remove();
                                deleteButton.remove();
                            })
                            .catch(error => {
                                console.error('There was a problem with the fetch operation:', error);
                            });
                    });
                }
            }
        });
    </script>

    <script>
        $(document).ready(function () {
            $('#small_image').dropify();
            $('#big_image').dropify();
        });
    </script>
@endsection